...
int read_simulation_file(const char *filename, SimulationParams *params, int max_params) {
    ...
    // Leer cada línea del archivo mientras no se alcance el final del archivo o el límite de simulaciones
    while (fgets(line, sizeof(line), file) && count < max_params) {
        ...
        int items_read = sscanf(line, "%s %lf %lf %lf %lf",
                                params[count].plate_file,   // Nombre del archivo de la placa
                                &params[count].dtime,       // Tiempo de cada etapa (Δt)
                                &params[count].alpha,       // Difusividad térmica (α)
                                &params[count].height,      // Tamaño de las celdas (h)
                                &params[count].epsilon);    // Criterio de convergencia (ε)

        // Si epsilon no se leyó (sólo se leyeron 4 valores), usar un valor predeterminado
        if (items_read < 5) {
