...
En la evaluación de desempeño entre OpenMP y Pthreads, se observaron mejoras significativas en las implementaciones concurrentes tras resolver el problema de distribución de datos. Las optimizaciones implementadas, como el mapeo por bloques y el uso de técnicas de padding para mitigar el *false sharing*, permitieron aprovechar de manera efectiva los recursos disponibles.
...
- *Pthreads*: Aunque más complejo de implementar, Pthreads ofreció un control más granular sobre los hilos. Esto permitió optimizar la distribución de la carga de trabajo y las sincronizaciones, logrando un rendimiento comparable al de OpenMP.

====== Rendimiento
- *OpenMP*: Gracias al balance automático de carga y a las mejoras en la distribución de datos, OpenMP alcanzó un desempeño cercano al óptimo en sistemas con varios núcleos. Sin embargo, su rendimiento dependió de la estrategia de paralelización y la correcta configuración de las directivas.
- *Pthreads*: Al gestionar manualmente la distribución de datos y evitar el *false sharing*, Pthreads logró un rendimiento igualmente competitivo. La implementación efectiva del mapeo por bloques permitió una distribución uniforme del trabajo, maximizando el uso de caché y reduciendo la latencia en las operaciones.

====== Escalabilidad
- *OpenMP*: Demostró una buena escalabilidad en sistemas con hasta 8 núcleos. Más allá de este punto, el impacto del *overhead* en la sincronización y las barreras comenzó a ser más significativo.
- *Pthreads*: Aunque más complejo de optimizar, Pthreads presentó una mejor escalabilidad en configuraciones con mayor cantidad de núcleos debido a su capacidad para personalizar la sincronización y minimizar el *overhead*.

====== Eficiencia General
Ambos enfoques lograron superar la implementación serial en escenarios donde el acceso a datos estaba bien distribuido y los hilos trabajaban sobre bloques independientes. Sin embargo, la diferencia clave radicó en la simplicidad y velocidad de implementación con OpenMP frente al control detallado y optimización manual que ofrece Pthreads.

En conclusión, OpenMP destacó por su facilidad de uso y rapidez de implementación, mientras que Pthreads, con un esfuerzo adicional en la programación, permitió exprimir al máximo el hardware disponible. Ambas herramientas, correctamente utilizadas, demostraron ser soluciones viables para mejorar el desempeño de la simulación concurrente.
