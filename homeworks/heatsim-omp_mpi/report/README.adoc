...
image::screen_shots/Desempeño-omp.png[comparación-desempeño]
...
En la evaluación de desempeño entre OpenMP y Pthreads, se observaron mejoras significativas en las implementaciones concurrentes tras resolver parcialmente algunos problemas de distribución de datos. Sin embargo, persistieron limitaciones en la implementación con Pthreads debido al manejo ineficiente de los hilos y problemas como el *false sharing*. Esto afectó negativamente el rendimiento de la simulación de transferencia de calor, especialmente en configuraciones de múltiples hilos.

====== Facilidad de Uso
- *OpenMP*: La abstracción que proporciona OpenMP resultó ser ventajosa, ya que simplificó la paralelización al reducir la cantidad de código necesario. Las directivas como `#pragma omp parallel for` y el manejo automático del balance de carga permitieron una implementación eficiente sin necesidad de gestionar manualmente la sincronización.
- *Pthreads*: Aunque más complejo de implementar, Pthreads ofreció un control más granular sobre los hilos. Sin embargo, en esta implementación particular, el rendimiento estuvo limitado por la falta de optimizaciones como el mapeo por bloque o el uso de un *thread pool*. Esto resultó en problemas de contención en la caché (*false sharing*), disminuyendo la eficiencia general del programa.

====== Rendimiento
- *OpenMP*: Gracias al balance automático de carga y la reducción del overhead asociado con la sincronización manual, OpenMP alcanzó un desempeño razonable, aunque todavía estuvo limitado por problemas inherentes al diseño concurrente, como el acceso a memoria compartida.
- *Pthreads*: Aunque teóricamente ofrece mayor control sobre la concurrencia, en este caso el rendimiento fue inferior debido a la falta de reutilización de hilos y el manejo ineficiente de datos compartidos. La ausencia de técnicas como el *padding* para mitigar el *false sharing* impactó significativamente la velocidad de ejecución.

====== Escalabilidad
- *OpenMP*: Mostró una buena escalabilidad en sistemas con hasta 12 núcleos, pero el rendimiento no escaló linealmente debido a problemas relacionados con el acceso a memoria compartida.
- *Pthreads*: La escalabilidad fue limitada por la contención en la caché y la sobrecarga causada por la creación y destrucción repetitiva de hilos. Estos factores hicieron que Pthreads no aprovechara de manera eficiente los recursos de hardware disponibles.
...
====== Conclusión
En conclusión, aunque la implementación con OpenMP mostró mejores resultados en términos de facilidad de uso y eficiencia general, la implementación con Pthreads presentó limitaciones significativas debido a la falta de optimizaciones. Sin embargo, con las mejoras sugeridas, la versión concurrente con Pthreads podría lograr un rendimiento significativamente mayor y aprovechar mejor los recursos de hardware disponibles.

==== Comparación de eficiencia

image::screen_shots/Comparacion-eficiencia-omp.png[comparación-eficiencia]
...
En conclusión, el grado de concurrencia influye significativamente en el desempeño y la eficiencia de una solución de software. A medida que se incrementa el número de hilos, el rendimiento no mejora de manera proporcional debido a problemas como el false sharing y la sobrecarga de sincronización. Sin embargo, este comportamiento adverso podría cambiar notablemente si se implementaran diferentes estrategias de mapeo, como el mapeo en bloque o mapeo en bloque cíclico, que permitirían distribuir mejor las cargas de trabajo entre los hilos y minimizar los conflictos de acceso a la memoria. Estas técnicas podrían reducir la interferencia entre hilos y mejorar tanto el rendimiento como la escalabilidad de la solución.