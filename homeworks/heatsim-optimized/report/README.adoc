...
|1 |Conc0 |9635.4943s | | |Versión concurrente inicial (Tarea02)
|2 |Conc1 |9334.2367s | | |Accerso a coef en memoria compartida
|3 |Conc2 |9932.9817s | | |Mapeo dinámico
|===
...
[[conc_iter02]] === Iteración 2: Implementación del Mapeo Dinámico con Contador de Filas Protegido
...
Mitigación propuesta: Para reducir el impacto del false sharing, se podrían explorar las siguientes soluciones:

Agregar padding (relleno) a las estructuras de datos compartidas para asegurarse de que las variables no se alineen en la misma línea de caché.
Cambiar el esquema de mapeo dinámico por otras estrategias como:
Mapeo por bloque: Asignar grupos consecutivos de filas a cada hilo, en lugar de asignar una fila a la vez.
Mapeo por bloque cíclico: Asignar las filas a los hilos de manera intercalada (por ejemplo, el hilo 1 procesa las filas 1, 4, 7, etc., y el hilo 2 procesa las filas 2, 5, 8, etc.). Esto puede reducir la probabilidad de que los hilos procesen filas adyacentes que comparten la misma línea de caché.
