...
En general, esta versión concurrente de la simulación de transferencia de calor muestra una buena gestión teórica de la paralelización y sincronización de los hilos, logrando un balance de carga dinámico y una correcta protección de datos compartidos. Sin embargo, durante la implementación, el rendimiento podría haberse visto afectado negativamente debido al false sharing, un problema común cuando múltiples hilos acceden a datos que residen en la misma línea de caché.
...
|===
|Versión |Descripción |
|SerialI |Versión serial inicial|
|SerialF |Versión serial final|
|ConcI |Versión concurrente inicial|
|Conc1 |Acceso a coef en memoria compartida|
|Conc2 |Mapeo dinámico|
|===
