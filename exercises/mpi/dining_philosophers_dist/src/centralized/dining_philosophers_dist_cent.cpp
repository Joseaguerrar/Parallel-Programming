...
void philosopher(int rank, int num_philosophers) {
    ...
    while (true) {
        ...
        sleep(rand() % 3 + 1);  // Pensar por un tiempo aleatorio //NOLINT
        ...
        MPI_Recv(NULL, 0, MPI_INT, num_philosophers, 2,
                                         MPI_COMM_WORLD, &status);  // GRANT = 2
        cout << "Filósofo " << rank << " está comiendo." << endl;
        sleep(rand() % 3 + 1);  // Comer por un tiempo aleatorio  //NOLINT

        // Liberar los palillos
        int release_msg[2] = {1, rank};  // RELEASE = 1
        MPI_Send(release_msg, 2, MPI_INT, num_philosophers, 0, MPI_COMM_WORLD);
        cout << "Filósofo " << rank
                << " ha terminado de comer y ha liberado los palillos." << endl;
    }
}
...
void waiter(int num_philosophers) {
    int *chopsticks = (int *)calloc(num_philosophers, sizeof(int));  //NOLINT
    // Estado de los palillos (0: libre, 1: ocupado)
    queue<pair<int, int>> request_queue;
    // Cola para solicitudes: (tipo, filósofo)
    MPI_Status status;
    ...
    while (true) {
        ...
        if (type == 0) {  // REQUEST
            request_queue.push({type, philosopher});
            // Agregar la solicitud a la cola
        } else if (type == 1) {  // RELEASE
            chopsticks[left] = 0;
            chopsticks[right] = 0;
            cout << "Mesero: Filósofo " << philosopher <<
                                           " ha liberado los palillos." << endl;
        }
        ...
        while (!request_queue.empty()) {
            ...
            if (chopsticks[req_left] == 0 && chopsticks[req_right] == 0) {
                ...
                cout << "Mesero: Otorgando permiso para comer al filósofo " <<
                                                 req_philosopher << "." << endl;
                MPI_Send(NULL, 0, MPI_INT, req_philosopher, 2, MPI_COMM_WORLD);
                // GRANT
                request_queue.pop();  // Eliminar la solicitud procesada
            } else {
                /* Si no hay palillos disponibles, salir del ciclo y
...
int main(int argc, char **argv) {
    ...
    if (size != num_philosophers + 1) {
        if (rank == 0) {
            cerr << "Error: Debes iniciar " << num_philosophers + 1
                      << " procesos (1 mesero + " << num_philosophers <<
                                                         " filósofos)." << endl;
        }
        MPI_Finalize();
        return EXIT_FAILURE;
    ...
    if (rank == num_philosophers) {
        ...
        // Los demás procesos son filósofos
    }
